   ...
   <footer class="mt-100 overflow-hidden footer-style-2">
                                    ...
                                    <ul class="footer-menu list-unstyled mb-0 d-md-block">
                                        @if(isset($category) != null && count($category) > 0)
                                            @foreach($category as $val)
                                                <li class="footer-menu-item"><a href="{{url('collection').'/'.$val->slug}}" class="text-white">{{$val->name}}</a></li>
                                            @endforeach
                                        @endif
                                    </ul>
                                </div>
                            ...
                            <div class="col-xl-4 col-lg-5 col-md-6 col-12 footer-widget">
                                <div class="footer-widget-inner">
                                    <h4 class="footer-logo">
                                        <a href="{{url('/')}}">
                                            <img src="{{url('asset/img/logo2.png')}}" loading="lazy" alt="bisum">
                                        </a>
                                    </h4>
                                    <div class="footer-newsletter">
                                        ...
                                        <div class="footer-social-wrapper">
                                            <ul
                                                ...
                                                <li class="footer-social-item text-white">
                                                    <a href="#">
                                            ...
                                            @if(isset($categories) && count($categories)>0)
                                                <ul class="main-menu list-unstyled">
                                                    @foreach($categories as $val)
                                                        <li class="menu-list-item nav-item has-megamenu">
                                                            <div class="mega-menu-header">
                                                                <a class="nav-link" href="{{url('collection').'/'.$val->slug}}">
                                                                    {{$val->name}}
                                                                </a>
                                                                <span class="open-submenu">
                                                                    <svg class="icon icon-dropdown" xmlns="http://www.w3.org/2000/svg" width="24"
                                                                    ...
                                                                    <ul class="submenu megamenu-container list-unstyled">
                                                                        @foreach($val->subcategories as $vals)       
                                                                            <li class="menu-list-item nav-item-sub">
                                                                                <div class="mega-menu-header">
                                                                                    <a class="nav-link-sub nav-text-sub megamenu-heading"
                                                                                    href="{{url('collections').'/'.$vals->slug}}"> {{$vals->name}}
                                                                                    </a>
                                                                                </div>
                                                                            </li>
                                                                        @endforeach
        ...
        <!-- Toggle Button for Help Section -->
        <!-- <button class="help-toggle-btn" onclick="toggleHelpSection()">Help</button> -->

        <!-- Help Section (Contact Form) -->
        <!-- <div class="contact-form-section help-section" id="helpSection">
          
            <div class="section-header mb-4">
                <h2 class=>Need Help?</h2>
                <p class="section-subheading">Send us a message.</p>
            </div>
                ...
                <form id="contactUs" class="contact-form">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12 col-12">
                            ...
                            <button type="button" onclick="formSubmit()" class="position-relative review-submit-btn contact-submit-btn">SEND MESSAGE</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <button id="quoteButton" class="quote-btn">Get Free Quote</button>

        <!-- Side Popup -->
        <div id="quotePopup" class="side-popup">
            <div class="popup-content">
                ...
                <form id="quote_from" class="contact-form row">
                    ...
                    <div class="col-md-12 col-12">
                        <fieldset>
                            <textarea cols="20" rows="3" placeholder="Write your message here" name="description" required>{{ old('description') }}</textarea>
                            @error('description')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </fieldset>
    ...
    <script>
        ...
        function formSubmit()
        {
            $.ajax({
                ...
                success: function(response) {
                    swal("Admin will connect within 24 hrs", "Message sent successfully!", "success")
                    $('#contactUs')[0].reset();
                },
                error: function(xhr) {
    ...
    <script>
        ...
        var cdivs = ["cMenu1", "cMenu2"];
        var visibleDivId = null; // Declare the variable

        function ctoggleVisibility(divId, divId1) {
                if (divId1 == 'cMenu11') 
                {
                    $('#cdisplay').html('Grid');
                    $('#cMenu21').removeClass('d-none');
                    $('#cMenu11').addClass('d-none');
                }
                else
                {
                    $('#cdisplay').html('List');
                    $('#cMenu21').addClass('d-none');
                    $('#cMenu11').removeClass('d-none');
                }
            if (visibleDivId === divId) {
                visibleDivId = null; // Hide all if the same div is clicked
            } else {
                visibleDivId = divId; // Set the clicked div as visible
            }
            chideNonVisibleDivs(); // Update visibility of divs
        }

        function chideNonVisibleDivs() {
            ...
            // If no div is visible, update display to "List"
            if (visibleDivId === null) {
                $('#cdisplay').html('List');
            }
        }

        var subcdivs = ["subcMenu1", "subcMenu2"];
        function subctoggleVisibility(divId, divId1) 
        {
            if (divId1 == 'subcMenu11') 
            {
                $('#subcdisplay').html('Grid');
                $('#subcMenu21').removeClass('d-none');
                $('#subcMenu11').addClass('d-none');
            }
            else
            {
                $('#subcdisplay').html('List');
                $('#subcMenu21').addClass('d-none');
                $('#subcMenu11').removeClass('d-none');
            }
            if(visibleDivId === divId) 
            {
                visibleDivId = null;
            ...
            subhideNonVisibleDivs();
        }

        function subhideNonVisibleDivs()
        {
            var i, divId, div;
            for(i = 0; i < divs.length; i++) 
            {
        ...
        var pdivs = ["pMenu1", "pMenu2"];
        function ptoggleVisibility(divId,divId1) 
        {
            if (divId1 == 'pMenu11') 
            {
                $('#pdisplay').html('Grid');
                $('#pMenu21').removeClass('d-none');
                $('#pMenu11').addClass('d-none');
            }
            else
            {
                $('#pdisplay').html('List');
                $('#pMenu21').addClass('d-none');
                $('#pMenu11').removeClass('d-none');
            }
            if(visibleDivId === divId) 
            {
                visibleDivId = null;
        ...
        $('#quote_from').on('submit', function(event) {
                event.preventDefault();
                ...
                $.ajax({
                    ...
                    data: $(this).serialize(),
                    success: function(response) {
                        swal("Sent", "Thank you for submitting!", "success");
                        document.getElementById('quote_from').reset();
                    },
                    error: function(xhr) {
                        let errorMessage = xhr.responseJSON.errors.email[0];
                        swal("Oops...", "Try Again", "error");
